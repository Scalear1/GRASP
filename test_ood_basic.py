...
def main():
    parser = argparse.ArgumentParser(description='Basic OOD Detection on TFinance Dataset')
    ...
    # Check if it's multiple runs or single run
    if isinstance(logits, list) and len(logits) > 0 and isinstance(logits[0], list):
        # Multiple runs format
        print(f"Logits loaded: list of {len(logits)} runs")
    else:
        # Single run format
        print(f"Logits loaded: {len(logits)} nodes x {len(logits[0])} classes")
